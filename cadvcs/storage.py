...
class BlobStore:
    ...
    def iter_digests(self):
        """Itera (digest, size) de todos los blobs del store."""
        for shard in self.root.iterdir():
            if not shard.is_dir():
                continue
            for obj in shard.iterdir():
                yield shard.name + obj.name, obj.stat().st_size
...
class S3BlobStore:
    ...
    def iter_digests(self):
        """Itera (digest, size) de todos los blobs bajo el prefijo."""
        ...
        for page in paginator.paginate(Bucket=self.bucket, Prefix=base):
            for obj in page.get("Contents", []):
                tail = obj["Key"][len(base):]          # ab/cdef...
                yield tail.replace("/", "", 1), obj["Size"]

    def delete(self, digest: str) -> None:
        self.s3.delete_object(Bucket=self.bucket, Key=self._key(digest))
